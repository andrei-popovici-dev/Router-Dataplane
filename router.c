...
int main(int argc, char *argv[])
{
	...
	rc = parse_arp_table("./arp_table.txt", arp_table); // remove later
	...
	while (1) {
		...
		if(ntohs(eth_hdr->ethr_type) == IP_ETHERTYPE) { //identificam tipul de pachet
			...
			ip_hdr = (struct ip_hdr*)(buf + sizeof(struct ether_hdr));
				...
				if(ip_hdr->dest_addr == get_interface_ip(interface)) { //daca destinatia suntem chiar noi atunci
				...
				ip_hdr->checksum = 0;
				if(ip_header_checksum != checksum(ip_hdr, sizeof(struct ip_hdr))){
				...
				//TODO: cautare in tabela de rutare Next Hop
